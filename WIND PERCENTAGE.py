...
# Linear regression model for wind energy prediction
...
model.fit(wind_speed, energy_production_mwh)

def predict_energy_production(wind_speeds_kph):
    # Predict energy production in MWh
    predictions = []
    for speed in wind_speeds_kph:
        if speed <= 7.2 or speed > 90:
            predictions.append(0)  # No energy production outside the specified range
        else:
            predictions.append(model.predict(np.array([[speed]]))[0])
    return np.array(predictions)

def calculate_wind_energy_percentage(month, day_type, wind_speeds_kph):
    # Get average consumption for the selected day
    avg_consumption = monthly_averages[month][day_type]
    
    # Predict energy production for each day
    predicted_productions = predict_energy_production(wind_speeds_kph)
    
    # Calculate percentage of demand met by wind energy
    percentages = [(production / avg_consumption) * 100 for production in predicted_productions]
    return percentages
def predict_energy_production(wind_speeds_kph):
    return model.predict(np.array(wind_speeds_kph).reshape(-1, 1))
...
def fetch_wind_forecast(location):
    try:
        ...
        hourly_wind_speeds = data['hourly']['wind_speed_10m']

        # Calculate daily average wind speeds
        daily_average_wind_speeds = []
        for i in range(0, len(hourly_wind_speeds), 24):
            daily_average = np.mean(hourly_wind_speeds[i:i+24])
            daily_average_wind_speeds.append(daily_average)
